...
begin
 {if FindWindow(nil, 'Документ - WordPad') <> 0 then
  ShowMessage('Нашел')
 else
  ShowMessage('Нету-с'); }
 SetWindowText(FindWindow(nil,'Документ - WordPad'),'test1');
end;

procedure TForm1.btn2Click(Sender: TObject);
var wd:HWND;
begin
  wd:=FindWindow(nil,'test1');
  if wd<>0 then
  begin
